...
This document details the design decisions, complexity analysis, and scaling strategies for a hierarchical vector retrieval system built for Novuz's CMaaP platform. The system combines metadata-aware tree routing with dual vector stores (FAQ and guardrails) at leaf nodes to provide accurate, scoped, and safe response generation.
...
**Overall: O(D·T + G·D + F·D) = O(D·(T + G + F))**

Breaking down each phase:

#### Phase 1: Tree Traversal — O(T)
...
**Distributed approach (recommended):**
```
Load balancer
    ↓
...
2. **Empirical threshold:**
   ...
   - Doesn't over-filter related FAQs (0.90 is still "related", not "same")
...
Rationale:
- Returning 2 high-quality FAQs > forcing 5 mediocre ones
